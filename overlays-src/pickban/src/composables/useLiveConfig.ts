...
// configures each broadcast in the website admin panel ("Live game" page): pick
// the two teams, click the players that actually play (handles subs), in role
// order (TOP→SUPPORT), and mark which team is on blue/red side. That config lives
// in public.live_config.
//
// The pick/ban overlay reads it and shows those names in fixed slots; champion
// picks come from BlueBottle, mapped by side + slot index:
//   blue slot i  ↔  blueTeam.slots[i]
//   red  slot i  ↔  redTeam.slots[i]
//
// The publishable key is safe to embed in a client-side overlay: access is
// bounded by the table's RLS policies (anon SELECT).
...
export function useLiveConfig(refreshMs = 5_000) {
  ...
  // public.matches so the overlay never depends on BlueBottle's scoreSeason,
  // which the operator would otherwise have to key in for every broadcast.
  const blueRecord = ref<TeamRecord>({ ...ZERO_RECORD })
  const redRecord  = ref<TeamRecord>({ ...ZERO_RECORD })
  // Coach names for the two teams (from public.teams.coach). Rendered above
  // the current-game ban row in the overlay as "COACH: <name>".
  const blueCoach = ref<string>('')
  const redCoach  = ref<string>('')

  async function fetchCoaches(blueId: number | null, redId: number | null): Promise<void> {
    const ids = [blueId, redId].filter((n): n is number => Number.isFinite(n))
    if (!ids.length) { blueCoach.value = ''; redCoach.value = ''; return }
    const r = await fetch(
      `${SUPABASE_URL}/rest/v1/teams?id=in.(${ids.join(',')})&select=id,coach`,
      { headers: HEADERS },
    )
    if (!r.ok) return
    const rows = await r.json() as Array<{ id: number; coach: string | null }>
    const byId = new Map<number, string>()
    for (const t of rows) byId.set(t.id, (t.coach ?? '').trim())
    blueCoach.value = blueId != null ? (byId.get(blueId) ?? '') : ''
    redCoach.value  = redId  != null ? (byId.get(redId)  ?? '') : ''
  }
  ...
  async function fetchOnce(): Promise<void> {
    try {
      ...
      if (!cfg) {
        ...
        blueRecord.value = { ...ZERO_RECORD }
        redRecord.value  = { ...ZERO_RECORD }
        blueCoach.value = ''; redCoach.value = ''
        return
      }
      ...
      await Promise.all([fetchRecords(blueTeamId, redTeamId), fetchCoaches(blueTeamId, redTeamId)])
    } catch (err) {
  ...
  return { blue, red, blueRecord, redRecord, blueCoach, redCoach }
}
